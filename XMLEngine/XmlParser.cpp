...
int main()
{
	...
	std::cout << "\n";	
	std::string src = "../ServerRepository/repository/Test_1_(time)/Test_1.xml";
	...
	std::cout << eles[0]->children()[0]->value() << std::endl;
	std::cout << eles[1]->children()[0]->value() << std::endl;
	pDoc->save("../ServerRepository/repository/xmlOutput.xml");

	XmlDocument newDoc;
	sPtr pRoot = makeTaggedElement("metadata");
	//XmlDocument doc(XmlProcessing::makeXmlDeclarElement());
	...
	docEle->addChild(XmlProcessing::makeCommentElement("metadata of package "));
	docEle->addChild(pRoot);
	XmlDocument doc(docEle);
	doc.xmlRoot(pRoot);

	sPtr child1 = makeTaggedElement("package");
	child1->addChild(makeTextElement("Test_1_time"));
	sPtr child2 = makeTaggedElement("user");
	child2->addChild(makeTextElement("client_1"));
	sPtr child3 = makeTaggedElement("timeStamp");
	child3->addChild(makeTextElement("time"));
	sPtr child4 = makeTaggedElement("dependency");
	sPtr child5 = makeTaggedElement("description");
	child5->addChild(makeTextElement("Description"));

	sPtr grandChild1 = makeTaggedElement("depPackage");
	grandChild1->addChild(makeTextElement("Test_2_time"));
	sPtr grandChild2 = makeTaggedElement("depPackage");
	grandChild2->addChild(makeTextElement("Test_3_time"));
	...
	std::cout << "\n" << doc.toString() << "\n\n";
	doc.save("../ServerRepository/repository/xmlOutput2.xml");
	//return doc;
	//std::ofstream outfile("xmlOutput.xml");
	//outfile << pDoc->toString() << std::endl;
	//outfile.close();
	...
	//std::cout << std::put_time(std::localtime(&now_c) ctime(&timev) << std::endl;
}
