...
using Util = Scanner::StringHelper;
template<typename T>
using Conv = Scanner::Converter<T>;
...
bool Socket::send(size_t bytes, byte* buffer)
{
	...
	while (bytesLeft > 0)
	{
		bytesSent = ::send(socket_, pBuf, (int)bytesLeft, 0);
...
bool Socket::recv(size_t bytes, byte* buffer)
{
	...
	while (bytesLeft > 0)
	{
		bytesRecvd = ::recv(socket_, pBuf, (int)bytesLeft, 0);
...
bool Socket::sendString(const std::string& str, byte terminator)
{
	...
	{
		bytesSent = (size_t)::send(socket_, pBuf, (int)bytesRemaining, 0);
...
{
	return (size_t)::send(socket_, pBuf, (int)bytes, 0);
}
...
{
	return (size_t)::recv(socket_, pBuf, (int)bytes, 0);
}
